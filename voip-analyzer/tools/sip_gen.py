...
def main():
    ...
    caller = sip_socket(SIP_PORT)
    callee = sip_socket(5061)  # callee's SIP UAS on a distinct port to avoid self-recv
    caller_rtp = sip_socket(CALLER_RTP)
    callee_rtp = sip_socket(CALLEE_RTP)

    for c in range(args.calls):
        cid = f"call{c}-{int(time.time())}@{CALLER_IP}"
        msgs = make_call(cid, from_tag=f"ft{c}", to_tag=f"tt{c}", branch=f"z9hG4bK{c}")
        print(f"[call {c}] Call-ID={cid}")
        for i, (direction, payload) in enumerate(msgs):
            data = payload.encode()
            if direction == "req":
            ...
            print(f"    -> {first_line}")
            # After the 200 OK (answer) and ACK, blast RTP
            if direction == "req" and payload.startswith("ACK") and args.rtp:
                for p in rtp_packets(args.rtp, ssrc=0x1111 + c):
                    caller_rtp.sendto(p, ("127.0.0.1", CALLEE_RTP))
                for p in rtp_packets(args.rtp, ssrc=0x2222 + c):
                    callee_rtp.sendto(p, ("127.0.0.1", CALLER_RTP))
                print(f"    .. sent {args.rtp}x2 RTP packets")
            time.sleep(args.delay)
        time.sleep(0.2)

    for s in (caller, callee, caller_rtp, callee_rtp):
        s.close()
    print("done.")
